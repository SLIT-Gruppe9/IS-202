...
public class AuthUser implements Serializable {
   ...
   @CollectionTable(name="authgroups",        // uppercase AUTHGROUPS fucks with case-sesnsitive OS ( glassfish,linux). <-- nope. error somewhere else.
...
//      this.password = AuthUtils.digest(password);
   ...
   // Returns primary group role (The first authgroup from DB, except admin ).
   // If user is member of student And teacher, the first one will be returned ( student ).
   // -eirik
   public String getMainGroup(){
       
       for(AuthGroup g : groups){
           if(!g.name().equals(("admin")))
               return g.name();
       }
       return "error";
   }
